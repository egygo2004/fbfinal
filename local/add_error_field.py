from appwrite.client import Client
from appwrite.services.databases import Databases
import time

ENDPOINT = "https://fra.cloud.appwrite.io/v1"
PROJECT_ID = "finalfb"
API_KEY = "standard_"
DB_ID = "fina"
COLL_QUEUE_ID = "numbers_queue"

def add_error_reason():
    client = Client()
    client.set_endpoint(ENDPOINT)
    client.set_project(PROJECT_ID)
    client.set_key(API_KEY)
    databases = Databases(client)
    
    print("[*] Adding error_reason attribute...")
    try:
        databases.create_string_attribute(
            database_id=DB_ID,
            collection_id=COLL_QUEUE_ID,
            key="error_reason",
            size=500,
            required=False
        )
        print("[+] error_reason attribute created. Waiting for availability...")
        time.sleep(5)
    except Exception as e:
        if "already exists" in str(e).lower():
            print("[i] error_reason already exists")
        else:
            print(f"[-] Error: {e}")

if __name__ == "__main__":
    add_error_reason()
