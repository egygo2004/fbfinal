...
const App = () => {
  ...
  const [historySearch, setHistorySearch] = useState(''); // For history search
  const [historyResults, setHistoryResults] = useState([]); // History search results
  ...
  const searchHistory = async () => {
    ...
    try {
      const res = await databases.listDocuments(DB_ID, QUEUE_COLL_ID, [
        Query.contains('phone', historySearch.trim()),
        Query.orderDesc('created_at'),
        Query.limit(100)
      ]);
      setHistoryResults(res.documents);
    } catch (e) {
      console.error('Error searching history:', e);
      // Fallback: try exact match
      try {
        const res = await databases.listDocuments(DB_ID, QUEUE_COLL_ID, [
          Query.equal('phone', historySearch.trim()),
          Query.limit(100)
        ]);
        setHistoryResults(res.documents);
      } catch (e2) {
        ...
        <header className="flex justify-between items-center mb-8 pb-6 border-b border-gray-800/50">
          ...
          <nav className="flex gap-2 bg-gray-900/50 p-1 rounded-xl border border-gray-800">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg font-medium transition-all ${activeTab === 'history' ? 'bg-emerald-500 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
            >
